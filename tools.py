...
class AnimateFieldDisplay:
    ...
    def __init__(self, dx, dt, maxXSize, minYSize, maxYSize, yLabel,
                 title=None):
        '''
    ...
    def drawBoundary(self, position):
        '''
    ...
    def updateData(self, data, timeCount):
        '''
        Обновить данные с распределением поля в пространстве
        '''
        self._line.set_ydata(data)
        time_str = '{:.5f}'.format(timeCount * self._dt * 1e9)
        self._ax.set_title('{} нс'.format(time_str))
        self._fig.canvas.draw()
        self._fig.canvas.flush_events()


def showProbeSignals(probes, dx, dt, minYSize, maxYSize):
    '''
    ...
    fig.canvas.set_window_title('Сигналы в датчиках')
